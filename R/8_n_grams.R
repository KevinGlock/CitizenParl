...
right_2grams <- partition(coi_r, year = 1996:1999) %>% # time span of interest
...
left_2grams <- partition(coi_l, year = 1996:1999) %>%
  partition(interjection = F) %>%
...
dt1 <- subset(dt1, dt1[["1_pos"]] == "ADJA") %>% subset(.[["2_pos"]] == "NN") # 2-gram ADJA-NN

dt1[,"1_pos" := NULL][,"2_pos" := NULL][,"exp_coi" := round(exp_coi, 2)][, "chisquare" := round(chisquare, 2)]
...
right_3grams <- partition(coi_r, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
  polmineR::ngrams(n = 3, p_attribute = c("word", "pos"))

left_3grams <- partition(coi_l, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
  polmineR::ngrams(n = 3, p_attribute = c("word", "pos"))
...
dt2 <- subset(dt2, dt2[["1_pos"]] == "NN") %>% subset(.[["2_pos"]] == "ART") #3-gram "NN", "NE", "ADJA", "CARD"
...
right_ngrams <- partition(coi_r, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
  polmineR::ngrams(n = 4, p_attribute = c("word", "pos"))

left_ngrams <- partition(coi_l, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
...
dt3 <- subset(dt3, dt3[["1_pos"]] == "ART") %>% subset(.[["2_pos"]] == "NN") %>% subset(.[["3_pos"]] == "APPR") # 4-gram "ART ; NN/ADJA" "APPR; ART/NN/NE/PIAT/PDAT/ADJA" "ADJA ; KON/NN" "$[ ; KON"

dt3[,"1_pos" := NULL][,"2_pos" := NULL][,"3_pos" := NULL][,"exp_coi" := round(exp_coi, 2)][, "chisquare" := round(chisquare, 2)] # format of table
...
right_5grams <- partition(coi_r, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
  polmineR::ngrams(n = 5, p_attribute = c("word", "pos"))

left_5grams <- partition(coi_l, year = 1996:1999) %>%
  partition(interjection = FALSE) %>%
  polmineR::ngrams(n = 5, p_attribute = c("word", "pos"))
